...
int	ofxFbole::_maxColorAttachments = -1;
int	ofxFbole::_maxDrawBuffers = -1;
int	ofxFbole::_maxSamples = -1;
...
void ofxFbole::checkGLSupport() {
	...
	ofLog(OF_LOG_NOTICE, string("ofxFbole::checkGLSupport()\n") +
...
void ofxFbole::setup(int width, int height, int internalformat, int numSamples) {
...
void ofxFbole::setup(Settings settings) {
	checkGLSupport();
...
GLuint ofxFbole::createAndAttachRenderbuffer(GLenum internalFormat, GLenum attachmentPoint) {
	GLuint buffer;
...
void ofxFbole::createAndAttachTexture(GLenum attachmentPoint) {
...
int ofxFbole::getNumTextures() {
	return textures.size();
}

ofTexture& ofxFbole::getTexture(int attachmentPoint) {
	updateTexture(attachmentPoint);
	return *textures.at(attachmentPoint);
}

void ofxFbole::updateTexture(int attachmentPoint) {
...
void ofxFbole::draw(float x, float y) {
	draw(x, y, settings.width, settings.height);
}


void ofxFbole::draw(float x, float y, float width, float height) {
	getTexture(0).draw(x, y, width, height);
}


GLuint ofxFbole::getFbo() {
	return fbo;
}

int ofxFbole::getWidth() {
	return settings.width;
}


int ofxFbole::getHeight() {
	return settings.height;
}


bool ofxFbole::checkStatus() {
